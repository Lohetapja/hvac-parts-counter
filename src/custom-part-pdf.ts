...
function firstPage(part: CustomPart): string {
  ...
  page += text(36, 547, `Part no: ${part.partNumber || '-'}   Type: ${part.partType}   Qty: ${part.quantity}`, 9);
  page += text(36, 532, `P1 ${profileLabel(part, 'a')}   P2 ${profileLabel(part, 'b')}   Length ${part.lengthMm} mm`, 9);
  page += text(36, 517, `Offsets X ${part.horizontalOffsetMm} mm / Y ${part.verticalOffsetMm} mm   Outlet H ${part.outletHorizontalAngleDeg} deg / V ${part.outletVerticalAngleDeg} deg / rotation ${part.outletRotationDeg} deg`, 8);
  ...
  page += text(36, 25, 'GEOMETRIC DESIGN AID - VERIFY ALL DIMENSIONS AND CONNECTION DIRECTIONS BEFORE FABRICATION.', 8, true); return page;
}
